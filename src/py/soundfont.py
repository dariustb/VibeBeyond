...
class SoundFont:
    ...
    def set_keys_name(self) -> str:
        ...
        if not get_sf2_names():
            raise ValueError(
                "No soundfonts found in src/assets/sf2/. View readme in folder to download soundfonts." # pylint: disable = line-too-long
            )
...
def get_sf2_names() -> tuple:
    ...
    names = [sf2 for sf2 in os.listdir(SF2_FOLDER) if sf2.lower().endswith(SF2_FILE_TYPE)]
    return tuple(names)
